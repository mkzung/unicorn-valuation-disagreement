...
`62b516d`. Re-running `python3 src/reproduce.py` on the extended data executes them unchanged;
...
That once is the commit named above. `astype(str)` rendered a missing value as the string
"nan" up to pandas 2 and leaves it missing from pandas 3, so `series_letters` handed a float
to a regex and the nightly job on the newest pandas the requirements allow died where the
pinned-environment job passed. `src/population.py` gains one helper that fills before the
cast, and thirteen call sites across three files use it. No test above changed, and the
reproduction run returns 571 canonical numbers with no drift: marks load through
`read_csv(dtype=str)`, which produces only `NaN`, and the helper writes exactly what pandas 2
wrote for it.
